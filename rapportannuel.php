...
$total_cash_query = $bdd->prepare("SELECT SUM(df.quanProd * df.prix) as totalCash
                                    ...
                                    WHERE YEAR(STR_TO_DATE(f.dateFac, '%d/%m/%Y')) = :year
                                    AND f.statut = 1 AND f.paie = 'cash'");
$total_cash_query->execute(['year' => $selected_year]);
...
$total_debt_query->execute(['year' => $selected_year]);
...
$total_items_query = $bdd->prepare("SELECT COUNT(*) as total 
                                    FROM factures 
                                    WHERE YEAR(STR_TO_DATE(dateFac, '%d/%m/%Y')) = :year 
                                    AND statut = 1");
$total_items_query->execute(['year' => $selected_year]);
            ...
            <div class="pagination">
                ...
                if ($total_pages > 1) {
                    for ($i = 1; $i <= $total_pages; $i++) {
                        ...
                        echo "<a href='?page=$i&year=$selected_year' class='$active_class'>$i</a>";
                    }
                }
                ?>
            </div>
