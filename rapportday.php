...
$total_cash_query = $bdd->prepare("SELECT SUM(df.quanProd * df.prix) as totalCash
                                    ...
                                    WHERE DATE(STR_TO_DATE(f.dateFac, '%d/%m/%Y')) = :date 
                                    AND f.statut = 1 AND f.paie = 'cash'");
$total_cash_query->execute(['date' => $selected_date]);
...
$total_debt_query->execute(['date' => $selected_date]);
...
$total_items_query = $bdd->prepare("SELECT COUNT(*) as total 
                                    FROM factures 
                                    WHERE DATE(STR_TO_DATE(dateFac, '%d/%m/%Y')) = :date 
                                    AND statut = 1");
$total_items_query->execute(['date' => $selected_date]);
...
$req->bindValue(':date', $selected_date, PDO::PARAM_STR);
        ...
        <div id="page-wrapper">
            ...
            <div class="date-selector">
                <form method="GET" action="">
                    ...
                    <input type="date" name="date" id="date" value="<?= $selected_date ?>" style="color: black;">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    if ($req->rowCount() > 0) {
                        ...
                        echo "<tr><td colspan='6' class='no-data'>Aucune donnée disponible pour cette date</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
            ...
            <div class="pagination">
                ...
                for ($i = 1; $i <= $total_pages; $i++) {
                    echo "<a href='?date=$selected_date&page=$i' class='" . ($page == $i ? 'active' : '') . "'>$i</a>";
                }
                ?>
            </div>
        </div>
